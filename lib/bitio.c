...
{
    int ret = read(reader->h.fd, data, len);
    if(ret>=0)
	reader->pos += ret;
    return ret;
}
...
{
    return lseek(r->h.fd, pos, SEEK_SET);
}
...
{
    return zzip_file_read(reader->h.ptr, data, len);
}
...
{
    return zzip_seek(reader->h.ptr, pos, SEEK_SET);
}
...
void writer_init_filewriter2(writer_t*w, char*filename)
{
    ...
    ((filewrite_t*)w->h.ptr)->free_handle = 1;
}

...
static char mymatrix_equals(const void*_m1, const void*_m2) {
    ...
    return !((*(U32*)&m1->m00 ^ *(U32*)&m2->m00)&0xfff00000) &&
...
#else
static int matrix_convert(gfxmatrix_t*in, const char*id, mymatrix_t*out, gfxmatrix_t*scalematrix, unsigned char alpha)
#endif
{
...
void fontfilter_countchar(fontfilter_t*f, gfxfont_t*font, int glyphnr, gfxcolor_t*color, gfxmatrix_t*matrix)
{
...
static void glyph_transform(gfxglyph_t*g, mymatrix_t*mm, double scale)
{
    gfxmatrix_t m;
    m.m00 = mm->m00 * scale;
    m.m01 = mm->m01 * scale;
...
static void fontfilter_freeze(fontfilter_t*f)
{
    internal_t*i = (internal_t*)f->internal;
	...
	font->glyphs = rfx_calloc(sizeof(gfxglyph_t)*(font->num_glyphs+1));
	count = 0;

	/* figure out a scaling value that causes the transformed glyphs to be exactly
	   1em (1024px) high */
	gfxmatrix_t gfxmatrix;
	gfxmatrix.m00 = fd->matrix.m00;
	gfxmatrix.m01 = fd->matrix.m01;
	gfxmatrix.m10 = fd->matrix.m10;
	gfxmatrix.m11 = fd->matrix.m11;
	gfxmatrix.tx = 0;
	gfxmatrix.ty = 0;
	gfxbbox_t tmp = {0,0,0,0};
	for(t=0;t<fd->orig->num_glyphs;t++) {
	    if(fd->used[t]) {
		gfxline_t*line = gfxline_clone(fd->orig->glyphs[t].line);
		gfxline_transform(line, &gfxmatrix);
		gfxbbox_t b = gfxline_getbbox(line);
		tmp = gfxbbox_expand_to_bbox(tmp, b);
		gfxline_free(line);
	    }
	}
	double height = tmp.ymax - tmp.ymin;
	double scale = 1.0;
	if(height>1e-5) {
	    scale = 1024.0 / height;
	}
	...
	for(t=0;t<fd->orig->num_glyphs;t++) {
		...
		glyph_transform(&font->glyphs[count], &fd->matrix, scale);
		fd->used[t] = count + 1;
		count++;
	    }
	}
...
void fontfilter_drawchar(fontfilter_t*f, gfxfont_t*font, int glyphnr, gfxcolor_t*_color, gfxmatrix_t*matrix, gfxdevice_t*out)
{
    internal_t*i = (internal_t*)f->internal;
    ...
    scalematrix.m00 *= d->scale;
    scalematrix.m01 *= d->scale;
    scalematrix.m10 *= d->scale;
    scalematrix.m11 *= d->scale;
    scalematrix.tx -= d->dx*scalematrix.m00;

    out->drawchar(out, d->font, d->used[glyphnr]-1, &color, &scalematrix);
}

static void fontfilter_destroy(fontfilter_t*f)
{
    internal_t*i = (internal_t*)f->internal;
    DICT_ITERATE_DATA(i->matrices, transformedfont_t*, fd) {
	if(fd->used) {
...
fontfilter_t* fontfilter_remove_font_transforms()
{
    ...
    fontfilter_t*f = (fontfilter_t*)rfx_calloc(sizeof(fontfilter_t));
    memset(f, 0, sizeof(fontfilter_t));
    
    f->countchar = fontfilter_countchar;
    f->freeze = fontfilter_freeze;
    f->drawchar = fontfilter_drawchar;
    f->destroy = fontfilter_destroy;

    i->matrices = dict_new2(&mymatrix_type);
    i->first = 1;
    i->marker_glyph = 0xF8FF;
    return f;
}
